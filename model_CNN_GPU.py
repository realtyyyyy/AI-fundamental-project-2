...
class FaceDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        face_tensor = face_tensor.type('torch.cuda.FloatTensor') # 指定为'torch.FloatTensor'型，否则送进模型后会因数据类型不匹配而报错
...
def train(train_dataset, val_dataset, batch_size, epochs, learning_rate, wt_decay):
    ...
    for epoch in range(epochs):
        ...
        for images, emotion in train_loader:
            images, emotion = images.to(device), emotion.to(device) # 将数据分配给GPU
            # 梯度清零
            optimizer.zero_grad()
            # 前向传播
...
def main():
    # 数据集实例化(创建数据集)
    train_dataset = FaceDataset(root='/root/autodl-tmp/facial-expression-recognition-main/face_images/train_set')
    val_dataset = FaceDataset(root='/root/autodl-tmp/facial-expression-recognition-main/face_images/verify_set')
    # 超参数可自行指定
    model = train(train_dataset, val_dataset, batch_size=128, epochs=100, learning_rate=0.1, wt_decay=0)
    # 保存模型
    torch.save(model, '/root/autodl-tmp/facial-expression-recognition-main/model/model_cnn.pkl')
